...
class MEXCClient(ExchangeInterface):
    """
    MEXC exchange client implementation.
    ...
    def __init__(self, config: Dict[str, str] = None, mock_mode: bool = None, account_name: str = None):
        ...
        super().__init__(exchange_name='mexc', config=config, mock_mode=mock_mode, account_name=account_name)
        self.exchange = None
    ...
    async def initialize(self) -> None:
        ...
        if self._initialized:
            logger.debug("MEXC client already initialized")
            return

        try:
            if self.mock_mode:
                logger.info(f"MEXC client ({self.account_name}) initialized in MOCK mode")
    ...
    async def get_balances(self) -> Dict[str, Dict[str, float]]:
        ...
        Returns:
            ...
            Example: {
                "USDT": {"free": 45000.0, "locked": 5000.0, "total": 50000.0},
                "ALKIMI": {"free": 1400000.0, "locked": 100000.0, "total": 1500000.0}
            }
        ...
        try:
            if self.mock_mode:
                logger.debug("Fetching MEXC balances (mock mode)")
                return get_mock_balances('mexc')
            ...
            logger.info(f"MEXC balances fetched: {balances}")
            return balances
    ...
    async def get_trades(self, since: datetime) -> List[Trade]:
        ...
        try:
            if self.mock_mode:
                logger.debug(f"Fetching MEXC trades since {since} (mock mode)")
                return get_cached_trades('mexc', since)
                ...
                for symbol in symbols_to_try:
                    try:
                        ...
                        while page_count < max_pages:
                            ...
                            logger.info(f"Page {page_count + 1}: Fetched {len(trades)} trades for {symbol}")

                            if not trades:
                                logger.info(f"No more trades found, stopping pagination")
            ...
            logger.info(f"MEXC trades fetched: {len(all_trades)} trades since {since}")
            return all_trades
    ...
    async def get_deposits(self, since: datetime) -> List[Transaction]:
        ...
        if self.mock_mode:
            return []  # Mock mode returns empty for now
        ...
        try:
            ...
            # Fetch deposits for tracked assets
            for asset in settings.tracked_assets:
                try:
            ...
            logger.info(f"MEXC deposits fetched: {len(all_deposits)} since {since}")
            return all_deposits

        except Exception as e:
            logger.error(f"Error fetching MEXC deposits: {e}")
            return []
    ...
    async def get_withdrawals(self, since: datetime) -> List[Transaction]:
        ...
        try:
            ...
            # Fetch withdrawals for tracked assets
            for asset in settings.tracked_assets:
                try:
            ...
            logger.info(f"MEXC withdrawals fetched: {len(all_withdrawals)} since {since}")
            return all_withdrawals

        except Exception as e:
            logger.error(f"Error fetching MEXC withdrawals: {e}")
            return []
    ...
    async def get_prices(self, symbols: List[str]) -> Dict[str, float]:
        ...
        try:
            if self.mock_mode:
                logger.debug(f"Fetching MEXC prices for {symbols} (mock mode)")
            ...
            for symbol in symbols:
                ...
                if not price_found:
                    logger.warning(f"Could not fetch price for {symbol} on MEXC, using 0.0")
                    prices[symbol] = 0.0

            logger.info(f"MEXC prices fetched: {prices}")
            return prices
    ...
    async def close(self) -> None:
        ...
        try:
            if self.exchange:
                ...
                logger.info("MEXC client closed successfully")
        except Exception as e:
